...
function formFromProgram(program: CatalogProgram): ProgramFormState {
	return {
		name: program.name,
		hubspotFormId: program.hubspotFormId,
...
function buildCreateBody(form: ProgramFormState): CreateCatalogProgramBody {
	const body: CreateCatalogProgramBody = {
		name: form.name.trim(),
		hubspotFormId: form.hubspotFormId.trim(),
	};
			...
			<div className={`modal ${styles.modal}`}>
				...
				<form className={styles.form} onSubmit={(event) => void handleSubmit(event)}>
					...
					<label>
						HubSpot form ID
						<input
							value={form.hubspotFormId}
							onChange={(event) => setForm((current) => ({ ...current, hubspotFormId: event.target.value }))}
							required
						/>
					</label>
					<label>
